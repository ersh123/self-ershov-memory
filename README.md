...
- [docs/release-integrity.md](docs/release-integrity.md) shows how to verify release assets, manifest, checksums, SBOM, and GitHub attestations.
...
- **Repo hygiene:** `uv.lock`, private security advisory path, issue/PR templates, CODEOWNERS, weekly Dependabot checks, pip-audit dependency vulnerability scanning, Ruff dead-code lint, OpenSSF Scorecard SARIF, ClusterFuzzLite fuzzing, and checkout-token hardening for GitHub Actions
...
The public release gate uses the fuller matrix in `docs/testing.md`: unit and CLI tests, property-based tests, fuzz harness smoke, plugin wrapper smoke, build/wheel/sdist smoke, docs guards, dependency vulnerability scanning, dead-code lint, CodeQL, ClusterFuzzLite, and scheduled-run soak evidence.
