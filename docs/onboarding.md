# Self Ershov Memory onboarding

Start here if you want the whole loop in one place.

Self Ershov Memory is a staged self-improvement loop. It scans explicit source inputs, stages proposed changes in an artifact directory, and only touches live state after an explicit apply step.
...
`~/.config/self-ershov-memory/nightly.secrets.env`; reinstalling the timer does not
touch that file.
...
```bash
ershov soak --state-root ~/.hermes/ershov --since-hours 30 --min-successful 1 --strict-systemd
```
...
The timer check requires an enabled, active, loaded timer pointing at `self-ershov-memory.service` with a next scheduled elapse.
...
When provider readiness is blocked, add `--require-provider deepseek --fix-plan` to `soak` or `status --release-gate` to print secret-safe remediation steps without changing files or printing key values.
The one-night command above is a fast release-candidate smoke. For public stable promotion, require several scheduled nights; plain `--strict-systemd` defaults to this gate:
