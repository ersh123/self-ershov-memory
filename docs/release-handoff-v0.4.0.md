# Self Ershov Memory v0.4.0 — Handoff
...
- `python scripts/generate_release_sbom.py --output dist/self-ershov-memory-sbom.spdx.json` (succeeds)
...
- Console packaging: wheel and source-distribution smokes cover the package-name `self-ershov-memory` alias in addition to `ershov` and legacy aliases, using `uv --no-cache` to avoid stale ephemeral envs
