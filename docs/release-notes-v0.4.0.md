# Self Ershov Memory v0.4.0 — Release Notes
...
- **`ershov providers list`** prints a table with `NAME`, `KIND`, `STATUS` (always | optional | missing), and `NOTES` for the built-in providers (`offline-marker`, `openai-compatible`, `deepseek`, `ollama`). No external services are pinged.
...
- The root Hermes plugin wrapper now propagates non-zero CLI failures, so `ershov ...` can be used as a real shell gate instead of only a human-readable wrapper.
...
- The publish workflow runs the same SBOM, release manifest, checksum, and verifier gates before uploading only wheel/source-distribution files to the PyPI publishing artifact.
- Wheel and source-distribution smoke tests now cover the package-name `self-ershov-memory` console alias in addition to `ershov` and legacy aliases, using `uv --no-cache` to avoid stale ephemeral envs.
- `docs/release-integrity.md` documents the public manifest, checksum, SBOM, `gh release verify-asset`, `gh attestation verify`, and stable-soak boundary for future GitHub Release consumers.
