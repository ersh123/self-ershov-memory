...
- Stable release wording waits for at least one real scheduled systemd run followed by a passing `hermes ershov soak --strict-systemd`. Manual service starts and transient timer smokes are useful evidence, but they are not the same as an overnight scheduled run.
