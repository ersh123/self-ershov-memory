...
- Wheel and source-distribution smoke tests now cover the package-name `hermes-ershov` console alias in addition to `ershov` and legacy aliases, using `uv --no-cache` to avoid stale ephemeral envs.
