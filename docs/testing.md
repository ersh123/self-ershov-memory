# Self Ershov Memory test matrix
...
uv run --locked --extra dev python -m pytest -q --cov=hermes_dreaming --cov=self_ershov_memory --cov=hermes_mnemos --cov-report=term-missing:skip-covered --cov-report=xml --cov-fail-under=80
...
ershov providers doctor --provider deepseek --env-file ~/.config/self-ershov-memory/nightly.env --env-file ~/.config/self-ershov-memory/nightly.secrets.env --strict
...
ershov status --release-gate --state-root /tmp/self-ershov-memory-state
ershov revert --help
```
...
- full pytest suite
- coverage report for `hermes_dreaming`, `self_ershov_memory`, and `hermes_mnemos`, with an 80% minimum gate
...
- installed wheel smoke for every public console alias (`ershov`, `self-ershov-memory`, `mnemos`, `nightmem`, `dreaming`) and module alias, with `uv --no-cache` so the check uses the freshly built artifact
- installed source distribution smoke for every public console alias (`ershov`, `self-ershov-memory`, `mnemos`, `nightmem`, `dreaming`) and module alias, with `uv --no-cache` so the check uses the freshly built artifact
...
- SPDX release SBOM generation into `dist/self-ershov-memory-sbom.spdx.json`
...
```bash
ershov soak --state-root ~/.hermes/ershov --since-hours 96 --min-successful 3 --strict-systemd
```
