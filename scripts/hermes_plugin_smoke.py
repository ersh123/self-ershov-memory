...
def _load_root_plugin():
    spec = importlib.util.spec_from_file_location("self_ershov_memory_root_plugin_smoke", ROOT / "__init__.py")
    ...
    try:
        ...
        conn.execute(
            ...
            (
                "smoke-1",
                "user",
                "MEMORY: memory: Keep Self Ershov Memory plugin smoke gates strict.",
                2_000.0,
            ),
        )
        conn.commit()
...
def _assert_nightly_stages_proposal(handler) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="self-ershov-memory-plugin-smoke-") as raw_tmp:
        tmp = Path(raw_tmp)
